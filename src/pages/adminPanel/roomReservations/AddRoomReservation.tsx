...
import { FormInputType, ValueType } from "../../../dataTypes/Input.type";
import {
  pastDateValidator,
...
export type DetailsDataType = {
  room: RoomDataType;
  price: string;
  reserved: RoomReservationDataType[];
  reservedDatesValues: DateObject[][];
  length: number;
  strength: ValueType;
  desc: string;
};
export default function AddRoomReservation() {
  ...
  const buttons: ButtonType[] = [
    {
      innerHtml: `${showDetails ? "مشاهده جزئیات" : "ثبت سفارش"}`,
      type: "submit",
  ...
  const submitHandler = async (items: FormValuesType) => {
    ...
    const reserved = roomReservations.filter(
      (item) =>
        item.roomID === roomID && intersection(reqDates, item.dates).length > 0
    );
    const desc =
      reserved.length > 0
        ? "اتاق مورد نظر در تاریخ های انتخاب شده پر است. برای انتخاب بهتر می توانید تاریخ هایی که اتاق رزرو شده است، از تقویم ببینید. "
        : "اتاق مورد نظر در روزهای انتخابی قابل رزرو است. با ثبت سفارش می توانید نسبت به رزرو اتاق اقدام کنید.";
    ...
    if (showDetails) {
      ...
      swal({
        text: `اتاق شماره ${room.roomNumber} از تاریخ ${enterDate} تا تاریخ ${exitDate} رزرو گردید.`,
        buttons: ["باشه"],
      });
